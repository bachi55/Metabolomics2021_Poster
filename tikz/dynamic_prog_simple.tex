...
\tikzstyle{places}=[circle,draw=blue!50,fill=blue!20,thick,minimum
   size=5mm]
\tikzstyle{placef}=[rectangle,draw=black!50,fill=gray!20,thick,minimum
...
\begin{tikzpicture}[every text node part/.style={align=center},inner sep=2mm]
  ...
  \draw[->,dotted,thick] (layer1) -- node[above] {Observed retention order (increasing retention
  time) $\longrightarrow$} (layerN) {};
  
  \node (row1) at (\xx+1.3*\xgap,\xy+2.5*\ygap+\ny*\ygap) {Highest MS/MS \\ based score};
  \node (rown) at
  (\xx+1.3*\xgap,\xy+2.5*\ygap) {Lowest MS/MS \\ based score};
  ...
  \draw[red,->,line width=1pt] (i_03) -- node[midway] {\large \color{blue} $\delta_{(1,1),(2,2)}$} (i_12) {};
  
  \draw[red,->,line width=3pt] (i_01) -- node[midway] {\large \color{blue} $\delta_{(1,3),(2,2)}$} (i_12) {};
  \draw[red,->,line width=3pt] (i_12) -- node[midway] {\large \color{blue} $\delta_{(2,2),(3,1)}$} (i_23) {};
  \draw[red,->,line width=3pt] (i_23) -- node[midway] {\large \color{blue} $\delta_{(3,1),(4,2)}$} (i_32) {};
  ...
  %\node (n_i) at (\xx+3.1*\xgap,\xy+1.75*\ygap) [placeo2] {\large
